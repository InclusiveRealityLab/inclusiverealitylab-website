...
function ButtonText({ label, linkAddress, className = "" }) {
  return (
    <Link
      to={linkAddress}
      className={`label w-btnW h-control rounded-sm flex items-center justify-center text-center text-text-primary hover:bg-element-secondaryLight/40 transition ease-in duration-200 ${className}`}
    >
      {label}
    </Link>
  );
